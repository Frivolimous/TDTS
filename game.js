...
var GameLayer;
(function (GameLayer) {
    GameLayer[GameLayer["BUILDINGS"] = 0] = "BUILDINGS";
    GameLayer[GameLayer["PROJECTILES"] = 1] = "PROJECTILES";
    GameLayer[GameLayer["ENEMIES"] = 2] = "ENEMIES";
    GameLayer[GameLayer["OVER"] = 3] = "OVER";
})(GameLayer || (GameLayer = {}));
var ObjectManager = (function () {
    function ObjectManager(_layer0) {
        var _this = this;
        this.layers = [];
        this.objects = [];
        this.objectsByType = [];
        ...
        this.layers.push(_layer0);
    }
    ObjectManager.prototype.addLayer = function (_layer) {
    ...
    ObjectManager.prototype.addObject = function (_object, _layer, _top) {
        if (_layer === void 0) { _layer = 0; }
        if (_top === void 0) { _top = true; }
        if (_top) {
            this.layers[_layer].addChild(_object);
        }
        else {
            this.layers[_layer].addChildAt(_object, 0);
        }
...
(function (ActivationType) {
    ActivationType[ActivationType["SHOT"] = 0] = "SHOT";
    ActivationType[ActivationType["NOVA"] = 1] = "NOVA";
})(ActivationType || (ActivationType = {}));
var BuildingData = (function () {
    function BuildingData() {
    }
    BuildingData.init = function () {
        this.buildings[BuildingType.BASIC] = { name: "Turret", texture: TextureData.mediumCircle, tint: 0x0000cc, subtype: BuildingType.BASIC, cost: 10,
            range: 100, damage: 5, maxCooldown: 50, activeType: ActivationType.SHOT, effects: [],
            goa: TwoOptions.Both, defaultPriority: PriorityType.FIRST };
        this.buildings[BuildingType.NOVA] = { name: "Nova", texture: TextureData.nova, tint: 0xcccc00, subtype: BuildingType.NOVA, cost: 20,
            range: 50, damage: 2, maxCooldown: 20, activeType: ActivationType.NOVA, effects: [],
            goa: TwoOptions.One, defaultPriority: PriorityType.ANY };
        this.buildings[BuildingType.ICE] = { name: "Slow", texture: TextureData.mediumCircle, tint: 0x00dddd, subtype: BuildingType.ICE, cost: 15,
            range: 70, damage: 4, maxCooldown: 70, activeType: ActivationType.SHOT, effects: [new EffectBuff(new SlowBuff(0.5, 100))],
            goa: TwoOptions.Both, defaultPriority: PriorityType.FIRST };
        this.buildings[BuildingType.POISON] = { name: "Poison", texture: TextureData.mediumCircle, tint: 0x007700, subtype: BuildingType.POISON, cost: 15,
            range: 90, damage: 2, maxCooldown: 50, activeType: ActivationType.SHOT, effects: [new EffectBuff(new PoisonBuff(1, 20, 100))],
            goa: TwoOptions.Both, defaultPriority: PriorityType.LAST };
        this.buildings[BuildingType.WALL] = { name: "Wall", texture: TextureData.wall, tint: 0x0000cc, subtype: BuildingType.WALL, cost: 2,
            defaultPriority: PriorityType.NONE };
    };
...
var PlayerModel = (function () {
    function PlayerModel() {
        this._Money = 200;
        this.sellPercent = 0.75;
        this._Lives = 20;
        this.moneyChangedEvent = new JMESelfRegister();
        this.livesChangedEvent = new JMESelfRegister();
        this.towers = [];
        this.towers = [
            BuildingData.buildings[BuildingType.WALL],
            BuildingData.buildings[BuildingType.BASIC],
            BuildingData.buildings[BuildingType.NOVA],
            BuildingData.buildings[BuildingType.ICE],
            BuildingData.buildings[BuildingType.POISON]
        ];
    }
    PlayerModel.prototype.addMoney = function (n) {
        this.money = this._Money + n;
    };
    Object.defineProperty(PlayerModel.prototype, "money", {
        get: function () {
            return this._Money;
        },
        set: function (n) {
            this._Money = n;
            this.moneyChangedEvent.publishEvent(new JME_MiscEvent(this._Money));
        },
        enumerable: true,
        configurable: true
    });
    PlayerModel.prototype.addLives = function (n) {
        this.lives = this._Lives + n;
    };
    Object.defineProperty(PlayerModel.prototype, "lives", {
        get: function () {
            return this._Lives;
        },
        set: function (n) {
            this._Lives = n;
            this.livesChangedEvent.publishEvent(new JME_MiscEvent(this._Lives));
        },
        enumerable: true,
        configurable: true
    });
...
(function (InteractionMode) {
    ...
    InteractionMode[InteractionMode["BUILD_1"] = 2] = "BUILD_1";
    InteractionMode[InteractionMode["BUILD_2"] = 3] = "BUILD_2";
    InteractionMode[InteractionMode["BUILD_3"] = 4] = "BUILD_3";
    InteractionMode[InteractionMode["BUILD_4"] = 5] = "BUILD_4";
    InteractionMode[InteractionMode["BUILD_5"] = 6] = "BUILD_5";
...
var GameManager = (function () {
    function GameManager(_app) {
                ...
                _this.objects.forEach(function () {
                    if (this.type == ObjectType.WALKING) {
                        ...
                        if (_object.currentTile != null && _object.currentTile.rating == 0) {
                            _object.goalEvent.publishEvent(new JME_MiscEvent(_object));
                        }
        ...
        this.canSpawn = true;
        this.enemyDead = function (e) {
            _this.earnMoney(e.value.worth, e.value.asPoint);
        };
        this.reachGoal = function (e) {
            e.value.toRemove = true;
            _this.player.addLives(-1);
            new JMFlyingText(String(-1), { fontSize: 12, fill: 0xff8888 }, e.value.asPoint, _this.gameStage);
        };
        ...
        this.onClick = function (e) {
            var _point = new PIXI.Point(e.mouse.x / _this.zoom - _this.gameStage.x, e.mouse.y / _this.zoom - _this.gameStage.y);
            if (_this.tilemap.bounds.contains(_point.x, _point.y)) {
                ...
                switch (_this.interactionMode) {
                    ...
                    case InteractionMode.SELL:
                        ...
                        if (_building != null) {
                            _this.earnMoney(Math.floor(_building.cost * _this.player.sellPercent), _this.tilemap.getTilePosition(_building.tile));
                            _this.objects.removeObject(_building);
                            _this.refreshAllWalkers();
                        }
                        break;
                    case InteractionMode.BUILD_1:
                    case InteractionMode.BUILD_2:
                    case InteractionMode.BUILD_3:
                    case InteractionMode.BUILD_4:
                    case InteractionMode.BUILD_5:
                        _building = _this.tilemap.getBuildingAt(_tile.index);
                        if (_building == null) {
                            _this.tryBuildObject(new BuildingObject(_tile, _this.player.towers[_this.interactionMode - InteractionMode.BUILD_1]), _tile);
                        }
                        else {
                            _this.gameUI.selectButtonAt(0);
                            _this.selectObject(_building);
                        }
                        break;
                }
        ...
        this.zoomIn = function () {
            _this.setZoom(_this.zoom * 1.1);
        };
        this.zoomOut = function () {
            _this.setZoom(_this.zoom * 0.9);
        };
        this.setZoom = function (n) {
            var maxZoom = 2;
            var minZoom = Math.min(1, Math.min(_this.playRect.width / _this.tilemap.bounds.width, _this.playRect.height / _this.tilemap.bounds.height));
            n = Math.min(maxZoom, Math.max(minZoom, n));
            _this.zoom = n;
            _this.gameStage.scale.set(n, n);
        };
        this.navigation = function () {
            ...
            var max = _this.playRect.x;
            var min = _this.playRect.right - _this.tilemap.bounds.width * _this.zoom;
            if (_this.gameStage.x < min) {
                _this.gameStage.x = min;
                _this.scroller.vX = 0;
            }
            if (_this.gameStage.x >= max) {
                _this.gameStage.x = max;
                _this.scroller.vX = 0;
            }
            max = _this.playRect.y;
            min = _this.playRect.bottom - _this.tilemap.bounds.height * _this.zoom;
            if (_this.gameStage.y < min) {
                _this.gameStage.y = min;
                _this.scroller.vY = 0;
            }
            if (_this.gameStage.y >= max) {
                _this.gameStage.y = max;
                _this.scroller.vY = 0;
            }
        };
        this.app = _app;
        ...
        this.tilemap.loadWalkMap("su-----------------------" +
            ...
            "uu-----------------------" +
            "uu-----------------------");
        this.waves.push(new WalkerWave([[100], [20, WalkingType.RUNNER, 5], [200], [20, WalkingType.FLYER, 10], [200], [20, WalkingType.RUNNER, 5]]));
        this.gameStage.addChildAt(this.tilemap, 0);
        this.gameUI = new GameUI;
        this.gameUI.addPlayer(this.player);
        this.gameUI.addSelections([["Select", InteractionMode.SELECT], ["Sell", InteractionMode.SELL],
            [this.player.towers[0].name, InteractionMode.BUILD_1], [this.player.towers[1].name, InteractionMode.BUILD_2],
            [this.player.towers[2].name, InteractionMode.BUILD_3], [this.player.towers[3].name, InteractionMode.BUILD_4],
            [this.player.towers[4].name, InteractionMode.BUILD_5]], function (i) { _this.interactionMode = i; });
    ...
    GameManager.prototype.trySpawn = function () {
        if (!this.canSpawn)
            return;
        if (this.waveIndex >= this.waves.length) {
            ...
            if (this.spawnTick < 0) {
                ...
                var _sprite = void 0;
                if (Math.random() < 0.7)
                    _sprite = new WalkingObject();
                else
                    _sprite = new FlyingObject();
                this.spawnWalker(_sprite, this.tilemap.spawns[Math.floor(Math.random() * this.tilemap.spawns.length)]);
            }
            return;
        }
        if (this.cWave == null) {
            if (this.waves[this.waveIndex].isDone()) {
                this.waveIndex += 1;
                if (this.waveIndex >= this.waves.length) {
                    this.canSpawn = null;
                }
            ...
            this.cWave = this.waves[this.waveIndex].getSpawnBlock();
        }
        if (this.cWave != null) {
            if (this.cWave.isDone()) {
                this.cWave = null;
            }
            else {
                var _toSpawn = this.cWave.getSpawn();
                if (_toSpawn != null) {
                    this.spawnWalker(_toSpawn, this.tilemap.spawns[Math.floor(Math.random() * this.tilemap.spawns.length)]);
                }
            }
        }
    };
    GameManager.prototype.spawnWalker = function (_walker, _tile) {
        _walker.currentTile = _tile;
        _walker.goto((_tile.loc.x + 0.5) * this.tilemap.tileSize, (_tile.loc.y + 0.5) * this.tilemap.tileSize);
        _walker.goalEvent.addListener(this.reachGoal);
        _walker.deathEvent.addListener(this.enemyDead);
        this.objects.addObject(_walker);
    };
    ...
    GameManager.prototype.earnMoney = function (_money, _loc) {
        this.player.addMoney(_money);
        if (_money > 0) {
            new JMFlyingText("+$" + String(Math.round(_money)), { fontSize: 12, fill: 0x007700 }, _loc, this.gameStage);
        }
        else if (_money < 0) {
            new JMFlyingText("-$" + String(Math.round(Math.abs(_money))), { fontSize: 12, fill: 0xff7700 }, _loc, this.gameStage);
        }
    };
    GameManager.prototype.tryBuildObject = function (_building, _tile) {
        if (this.player.money < _building.cost)
            return;
        if (this.tilemap.addBuildingAt(_building, _tile.index)) {
            this.earnMoney(-_building.cost, this.tilemap.getTilePosition(_tile));
...
var GameUI = (function (_super) {
    ...
    function GameUI() {
        var _this = _super.call(this) || this;
        _this.selectButtons = [];
        _this.updateMoney = function () {
            _this.topBar.moneyText.text = String(_this.player.money);
        };
        _this.updateLives = function () {
            _this.topBar.livesText.text = String(_this.player.lives);
        };
        ...
        _this.topBar = new TopBar({ x: 0, y: 0, width: facade.stageBorders.width - 150, height: 15, bgColor: 0xcccccc, alpha: 0.5 });
    ...
    GameUI.prototype.addPlayer = function (_player) {
        this.clearPlayer();
        this.player = _player;
        this.player.moneyChangedEvent.addListener(this.updateMoney);
        this.player.livesChangedEvent.addListener(this.updateLives);
        this.updateMoney();
        this.updateLives();
    };
    GameUI.prototype.clearPlayer = function () {
        if (this.player != null) {
            this.player.moneyChangedEvent.removeListener(this.updateMoney);
            this.player.livesChangedEvent.removeListener(this.updateLives);
        }
    };
...
var TopBar = (function (_super) {
    ...
    function TopBar(_options) {
        var _this = _super.call(this, _options) || this;
        var _text = new PIXI.Text("Money: ", { fontSize: 12, fontWeight: 'bold', fill: 0xffff99 });
        _text.y = 0;
        _text.x = _this.graphics.width - 105;
        _this.addChild(_text);
        _this.moneyText = new PIXI.Text("0000", { fontSize: 12, fill: 0xffff99 });
        _this.moneyText.y = 0;
        _this.moneyText.x = _this.graphics.width - 60;
        _this.addChild(_this.moneyText);
        _text = new PIXI.Text("Lives: ", { fontSize: 12, fontWeight: 'bold', fill: 0xff8888 });
        _text.y = 0;
        _text.x = _this.graphics.width - 205;
        _this.addChild(_text);
        _this.livesText = new PIXI.Text("0000", { fontSize: 12, fill: 0xff8888 });
        _this.livesText.y = 0;
        _this.livesText.x = _this.graphics.width - 160;
        _this.addChild(_this.livesText);
        return _this;
    }
    return TopBar;
}(JMBUI_BasicElement));
var PropertiesPanel = (function (_super) {
    ...
    PropertiesPanel.prototype.showBuildingProperties = function (_building) {
        var _title = new PIXI.Text(_building.name, PropertiesPanel.TitleStyle);
        ...
        if (_building.level < _building.maxLevel) {
            var _button = new JMBUI_Button({ x: 50, y: this.height - 30, width: 100, height: 20, label: "Upgrade ($" + _building.cost + ")", output: _building.levelup });
            this.addChild(_button);
        }
    };
...
var DragItem = (function (_super) {
    ...
    function DragItem() {
        var _this = _super.call(this, TextureData.itemRect) || this;
        _this.draggable = true;
        _this.buttonMode = true;
        _this.interactive = true;
        return _this;
...
var Facade = (function () {
    ...
    Facade.prototype.diminish = function (n, p, i) {
        return n * Math.pow(1 - p, i - 1);
    };
    Facade.prototype.compound = function (n, p, i) {
        return n * Math.pow(1 + p, i - 1);
    };
    Facade.exists = false;
    return Facade;
...
var JMFlyingText = (function (_super) {
    __extends(JMFlyingText, _super);
    function JMFlyingText(s, _style, _loc, _parent) {
        if (_parent === void 0) { _parent = null; }
        var _this = _super.call(this, s, JMBL.applyDefaultOptions(_style, { fontSize: 15, fontWeight: 'bold', dropShadow: true, fill: 0xffffff, dropShadowDistance: 2 })) || this;
        _this.x = _loc.x - _this.width / 2;
        _this.y = _loc.y;
        if (_parent != null)
            _parent.addChild(_this);
        JMBL.tweenTo(_this, 60, { delay: 20, alpha: 0 });
        JMBL.tweenTo(_this, 80, { y: (_this.y - 20) }, _this.destroy);
        return _this;
    }
    return JMFlyingText;
}(PIXI.Text));
...
(function (ObjectEvent) {
    ObjectEvent[ObjectEvent["MISC"] = 0] = "MISC";
    ObjectEvent[ObjectEvent["END_TWEEN"] = 1] = "END_TWEEN";
...
var JMESelfRegister = (function (_super) {
    __extends(JMESelfRegister, _super);
    function JMESelfRegister(_type) {
        if (_type === void 0) { _type = null; }
        return _super.call(this, _type) || this;
    }
    JMESelfRegister.prototype.addListener = function (_function) {
        this.listeners.push(_function);
    };
    JMESelfRegister.prototype.removeListener = function (_function) {
        for (var i = 0; i < this.listeners.length; i += 1) {
            if (this.listeners[i] == _function) {
                this.listeners.splice(i, 1);
            }
        }
    };
    JMESelfRegister.prototype.publishEvent = function (_event) {
        if (_event === void 0) { _event = null; }
        JME.publishSelfEvent(this, _event || new JMEvent());
    };
    return JMESelfRegister;
}(JMERegister));
...
var JME_MiscEvent = (function (_super) {
    __extends(JME_MiscEvent, _super);
    function JME_MiscEvent(_value) {
        var _this = _super.call(this) || this;
        _this.value = _value;
        return _this;
    }
    return JME_MiscEvent;
}(JMEvent));
...
var JME_KeyEvent = (function (_super) {
    ...
    function JME_KeyEvent(_key) {
        var _this = _super.call(this) || this;
        _this.eventType = EventType.KEY_DOWN;
        _this.key = _key;
        return _this;
    }
...
var JMTileMap = (function (_super) {
    ...
    function JMTileMap(options) {
        ...
        _this.tiles = [];
        _this.spawns = [];
        _this.getTilePosition = function (_tile, _outside) {
            if (_outside === void 0) { _outside = false; }
            var m = new PIXI.Point((_tile.loc.x + 0.5) * _this.tileSize, (_tile.loc.y + 0.5) * _this.tileSize);
            if (_outside) {
                m.x += _this.x;
                m.y += _this.y;
            }
...
var BuildingObject = (function (_super) {
    ...
    function BuildingObject(_tile, _options) {
        if (_tile === void 0) { _tile = null; }
        if (_options === void 0) { _options = {}; }
        var _this = _super.call(this, _options.texture || TextureData.mediumCircle) || this;
        _this.speed = 4;
        _this.cooldown = 50;
        ...
        _this.maxLevel = 5;
        _this.levelData = {
            linear: { maxCooldown: -1, range: 5, damage: 0.5 },
            diminish: {},
            compound: { cost: 10 },
            2: { maxCooldown: -10, range: 10, damage: 3 }
        };
        _this.levelup = function (_numLevels) {
            if (_numLevels === void 0) { _numLevels = 1; }
            _this.level += 1;
            _this.applyLevelStats(_this.levelData, _this.level);
            if (_numLevels > 1) {
                _this.levelup(_numLevels - 1);
            }
        };
        _this.activate = function (manager) {
            if (_this.activeFunction == null)
                return;
            if (_this.cooldown < 0) {
        ...
        _this.tile = _tile;
        _this.range = _options.range || 0;
        _this.damage = _options.damage || 0;
        _this.cooldown = _this.maxCooldown = _options.maxCooldown || 0;
        _this.subtype = _options.subtype || 0;
        _this.setTint(_options.tint || 0x0000cc);
        _this.assignActiveType(_options.activeType);
        _this.goa = _options.goa != null ? _options.goa : TwoOptions.Both;
        _this.cost = _options.cost || 0;
        _this.setPriority(_options.defaultPriority || 0);
        _this.effects = _options.effects || [];
        return _this;
    }
    BuildingObject.prototype.assignActiveType = function (_type) {
        switch (_type) {
            case ActivationType.SHOT:
                this.activeFunction = this.shootProjectile;
                break;
            case ActivationType.NOVA:
    ...
    BuildingObject.prototype.applyLevelStats = function (_levelData, _level) {
        if (_levelData.linear != null) {
            for (var v in _levelData.linear) {
                this[v] += _levelData.linear[v];
            }
        }
        if (_levelData.compound != null) {
            for (var v in _levelData.compound) {
                this[v] += facade.compound(_levelData.compound[v], 0.2, _level);
            }
        }
        if (_levelData.diminish != null) {
            for (var v in _levelData.diminish) {
                this[v] += facade.diminish(_levelData.diminish[v], 0.2, _level);
            }
        }
        if (_levelData[_level] != null) {
            for (var v in _levelData[_level]) {
                this[v] += _levelData[_level][v];
            }
        }
    };
...
var WalkerWave = (function () {
    function WalkerWave(spawns) {
        this._Waves = [];
        this._Count = 0;
        this.currentWave = 0;
        for (var i = 0; i < spawns.length; i += 1) {
            if (spawns[i].length == 1) {
                this._Waves.push(new SpawnBlock(spawns[i][0]));
            }
            else {
                this._Waves.push(new SpawnBlock(spawns[i][0], spawns[i][1], spawns[i][2]));
            }
        }
    }
    WalkerWave.prototype.isDone = function () {
        if (this.currentWave >= this._Waves.length)
            return true;
        return false;
    };
    WalkerWave.prototype.getSpawnBlock = function () {
        if (this.isDone())
            return null;
        var _wave = this._Waves[this.currentWave];
        if (_wave == null) {
            return null;
        }
        else if (_wave.type == -1) {
            this._Count += 1;
            if (this._Count >= _wave.delay) {
                this.currentWave += 1;
                return this.getSpawnBlock();
            }
            else {
                return null;
            }
...
var SpawnBlock = (function () {
    function SpawnBlock(delay, type, count) {
        if (type === void 0) { type = -1; }
    ...
    SpawnBlock.prototype.isDone = function () {
        if (this.type < 0) {
            return true;
        }
        else {
    ...
    SpawnBlock.prototype.getSpawn = function () {
        if (this.type < 0)
            return null;
        this._Delay += 1;
        if (this._Delay >= this.delay) {
            this.count -= 1;
            this._Delay = 0;
            return WalkingObject.walkingFromType(this.type);
        }
        else {
            return null;
        }
...
var WalkingObject = (function (_super) {
    ...
    function WalkingObject(_tile, _texture, _tint) {
        ...
        _this.worth = 1;
        _this.deathEvent = new JMESelfRegister;
        _this.goalEvent = new JMESelfRegister;
        _this.killMe = function () {
            ...
            _this.toRemove = true;
            _this.deathEvent.publishEvent(new JME_MiscEvent(_this));
        };
        ...
        _this.update = function () {
            ...
            var _static = true;
            var diffX = _this.goal.x - _this.x;
            var diffY = _this.goal.y - _this.y;
            var dTotal = Math.abs(diffX) + Math.abs(diffY);
            if (diffX != 0) {
                if (Math.abs(diffX) < 1) {
                    _this.x;
                    _this.goal.x;
                    ...
                    _this.x += (diffX / dTotal) * _this.speed * (_this.currentTile ? _this.currentTile.speed : 1);
                    _static = false;
                    _this.moving = true;
                }
            }
            if (diffY != 0) {
                if (Math.abs(diffY) < 1) {
                    ...
                    _this.y += (diffY / dTotal) * _this.speed * (_this.currentTile ? _this.currentTile.speed : 1);
                    _static = false;
                    _this.moving = true;
                }
            }
        ...
        _this.subtype = WalkingType.RUNNER;
        _this.worth = 1;
        _this.speed = 1.25 * (0.9 + Math.random() * 0.2);
        return _this;
    }
    ...
    WalkingObject.walkingFromType = function (_type, _tile) {
        if (_tile === void 0) { _tile = null; }
        switch (_type) {
...
var FlyingObject = (function (_super) {
    ...
    function FlyingObject(_tile) {
        ...
        _this.update = function () {
            ...
            if (diffX != 0) {
                if (Math.abs(diffX) < 1) {
                    ...
                    _this.x += (diffX / dTotal) * _this.speed;
                    _static = false;
                    _this.moving = true;
                }
            }
            if (diffY != 0) {
                if (Math.abs(diffY) < 1) {
                    ...
                    _this.y += (diffY / dTotal) * _this.speed;
                    _static = false;
                    _this.moving = true;
                }
            }
        ...
        _this.worth = 2;
        _this.speed = 1 * (0.9 + Math.random() * 0.2);
        return _this;
    }
